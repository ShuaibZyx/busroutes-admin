<template>
  <el-card shadow="never">
    <div slot="header" class="cardHeader">
      <el-page-header @back="$router.back()" content="创建线路" />
    </div>
    <div class="createRoute">
      <div class="common">
        ...
        <div class="btns">
          ...
          <el-button type="primary" size="small" @click="createRoute">
            创建
          </el-button>
        </div>
      </div>
  ...
  methods: {
    ...
    createRoute() {
      this.$refs.form.validate(async (valid) => {
        ...
        const { data: createRouteRes } = await this.$axios.post(
          "route/create",
          this.route
        );
        this.resetForm();
        this.$message({
          message: createRouteRes.msg,
          type: `${createRouteRes.code !== 200 ? "error" : "success"}`,
          center: true,
        });
      });
    },
